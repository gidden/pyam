...
def test_op_data():
    ...
    axis = 'variable'
    kwargs = {
        'model': 'x', 'year': 2010, 'region': 'y', 'unit': 'z', 
        'scenario': 'a',
    }
    a = IamDataFrame(pd.DataFrame({
        'value': [1,],
        axis: ['var1',],
        **kwargs,
    }))
    b = IamDataFrame(pd.DataFrame({
        'value': [2,],
        axis: ['var2',],
        **kwargs,
    }))
    ...
    idx = index=pd.MultiIndex.from_tuples([kwargs.values(),], names=names)

    obs_a.index = obs_a.index.reorder_levels(names)
    exp_a = pd.Series([1.,], name='value', index=idx)
    assert_series_equal(obs_a, exp_a)

    obs_b.index = obs_b.index.reorder_levels(names)
    exp_b = pd.Series([2.,], name='value', index=idx)
    assert_series_equal(obs_b, exp_b)
